...
$$
MRR(T_{test}) = \frac{1}{|T_{test}|} \sum_{(h^+, r, t^+) \in T_{test}} \frac{1}{rank(h^+, r, t^+)}
$$
...
$$
d(L_i, L_j) = \sqrt{\sum_{k=1}^{g} (V_{ki} - V_{kj})^2}
$$

为了计算的统一性，本文对其进行归一化处理：

$$
Sim_{KG}(i, j) = \frac{d(L_i, L_j) - \mu(d)}{\sigma(d)}
$$
...
$$
\mu(d) = \frac{1}{T} \sum_{r=1}^{T} d(L_i, L_r)
$$

$$
\sigma^2(d) = \frac{1}{T} \sum_{r=1}^{T} (d(L_i, L_r) - \mu(d))^2
$$
...
$$
Sim_{CF}(i, j) = \cos(\vec{L_i}, \vec{L_j}) = \frac{\vec{g_i} \cdot \vec{g_j}}{\|\vec{g_i}\| \times \|\vec{g_j}\|} = \frac{\sum_{k=1}^{S} T_{ki} \times T_{kj}}{\sqrt{\sum_{k=1}^{S} T_{ki}^2} \times \sqrt{\sum_{k=1}^{S} T_{kj}^2}}
$$
