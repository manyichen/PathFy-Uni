...
async function search() {
  ...
  try { candidates.value = (await api.ok<any>('/api/report/targets/manual-search', { method: 'POST', body: { q: query.value, limit: 20 } })).targets || [] }
  catch (error) { toast.add({ title: error instanceof Error ? error.message : '搜索失败', color: 'error' }) }
  finally { searching.value = false }
}

function addTarget(item: any) {
  if (targets.value.length >= 5) { toast.add({ title: '最多选择 5 个目标', color: 'warning' }); return }
  if (!targets.value.some(x => x.job_id === item.job_id)) targets.value.push(item)
  persist()
}
...
onMounted(async () => {
  ...
  if (body.code === 200) { resumes.value = body.data || []; resumeId.value = resumes.value[0]?.id }
  ...
  <div class="page-stack">
    ...
    <UCard>
      <div class="report-config">
        <section class="config-block"><p class="config-label">1 · 选择来源</p><div class="flex flex-wrap gap-2"><UButton icon="i-lucide-wand-sparkles" :loading="loading === 'import'" @click="openMatchHistory">导入匹配数据</UButton><UButton color="neutral" variant="soft" icon="i-lucide-search" @click="query ||= '开发'; search()">搜索岗位</UButton></div><p class="mt-2 text-xs muted">推荐从人岗匹配历史导入，画像和目标策略会自动关联。</p></section>
        ...
        <section class="config-block"><div class="flex items-center justify-between"><p class="config-label">3 · 已选目标 {{ targets.length }}/5</p><UButton v-if="targets.length" size="xs" color="neutral" variant="ghost" @click="targets = []">清空</UButton></div><div class="flex flex-wrap gap-1.5"><UBadge v-for="(item, i) in targets" :key="item.job_id" :label="item.title || item.job_id" class="cursor-pointer" @click="removeTarget(Number(i))" /><span v-if="!targets.length" class="text-sm muted">尚未选择目标</span></div></section>
