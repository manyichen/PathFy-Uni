...
const mode = ref('merge'); const sourceId = ref(''); const batch = ref(128); const promotions = ref(true); const lateral = ref(true)
...
const contracts: Partial<Record<GraphTaskType,string>> = {
  job_import: '招聘岗位 Excel；新增或变化岗位会强制完成八维评分，岗位名称同步不可关闭。',
  job_capability_result_import: '每行一个岗位评估 JSON，必须包含 job_key/job_id、八维分数和八维置信度。',
  learning_resource_import: 'master/learning_resources.csv 字段契约。', competition_import: 'master/competitions.csv 字段契约。',
  job_promotion_import: 'promotion_id、job_title、title、promotion、stage1-3 等字段。',
  job_lateral_import: 'from_job_title、to_job_title、score、rank、track、cap_similarity 等字段。',
  promotion_recommendation_import: '同时上传学习资源推荐与竞赛推荐 CSV，两部分作为一个事务确认。'
}
...
async function submit() {
  ...
  try {
    ...
    if (Object.values(files).some(Boolean)) {
      ...
      form.append('batch_size', String(batch.value)); form.append('generate_promotions', String(promotions.value)); form.append('generate_lateral', String(lateral.value)); form.append('scope', scope.value); form.append('force', String(force.value))
      Object.entries(files).forEach(([role,file]) => { if (file) form.append(role, file) }); body = form
    } else body = { task_type: type.value, mode: mode.value, source_id: sourceId.value, scope: scope.value, force: force.value }
...
<template>
  <div class="page-stack">
    ...
    <div class="page-heading"><h1>更新图谱</h1><p class="muted">先生成完整变更集，确认前 Neo4j 不会变化</p></div>
    <div class="grid gap-5 lg:grid-cols-[18rem_minmax(0,1fr)]">
      ...
      <UCard><template #header><div class="flex items-center gap-3"><UIcon :name="selected.icon" class="size-6 text-primary"/><div><h2 class="font-semibold">{{ selected.label }}</h2><p class="text-sm muted">{{ selected.category }}</p></div></div></template>
        <div class="grid gap-5 md:grid-cols-2">
          ...
          <template v-if="type === 'job_import'"><UFormField label="LLM 批大小"><UInput v-model.number="batch" type="number" min="1" max="1000"/></UFormField><div class="grid gap-2"><UCheckbox v-model="promotions" label="生成自动晋升路线"/><UCheckbox v-model="lateral" label="生成自动换岗关系"/><p class="text-xs muted">岗位名称同步和八维评估始终执行。</p></div></template>
          <template v-if="type === 'job_capability_evaluation'"><UFormField label="评估范围"><USelect v-model="scope" :items="[{label:'仅缺失评分',value:'missing'},{label:'版本过期',value:'stale'},{label:'全部岗位',value:'all'}]" class="w-full"/></UFormField><UFormField label="限定 source_id（可选）"><UInput v-model="sourceId"/></UFormField></template>
          <UCheckbox v-if="type === 'salary_normalization'" v-model="force" label="强制重新计算所有岗位"/>
        </div>
        <UAlert v-if="contracts[type]" class="mt-5" icon="i-lucide-info" title="输入契约" :description="contracts[type]"/>
